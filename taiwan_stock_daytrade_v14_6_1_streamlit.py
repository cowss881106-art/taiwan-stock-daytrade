...
def v14_scan(result_df):
    """v14.6.1：09:30 前不鎖池；ORB 完成後第一次全市場建池，同日後續只刷新固定候選池。"""
    stock_pool = load_stock_pool()
    ...
    # v14.6.1 安全鎖：09:30 ORB 尚未完成前，不建立/鎖定候選池。
    # 交易日以 Yahoo 最新可用 5 分 K 決定；若是今天，另要求台灣時間已到 09:30。
    bench = v14_prepare_intraday(v14_download_5m(V14_BENCHMARK), trade_date=trade_date)
    latest_5m_ts = bench.index[-1] if not bench.empty else None
    now_tw = datetime.now(ZoneInfo("Asia/Taipei"))
    is_today = trade_date == now_tw.date()
    data_orb_ready = latest_5m_ts is not None and latest_5m_ts.time() >= dt_time(9, 25)
    wallclock_ready = (not is_today) or now_tw.time() >= dt_time(9, 30)
    orb_ready_to_lock = data_orb_ready and wallclock_ready

    if not orb_ready_to_lock:
        # 若同交易日曾意外留下未完成的池，先清掉，避免後續誤用。
        if st.session_state.get("v14_pool_trade_date") == str(trade_date):
        ...
        latest_txt = latest_5m_ts.strftime("%H:%M") if latest_5m_ts is not None else "無資料"
        regime.update({
            "pool_ready":False, "long_pool_n":0, "short_pool_n":0,
            "union_pool_n":0, "scan_n":0, "scan_mode":"ORB尚未完成，暫不鎖定候選池",
            "latest_5m_time":latest_txt
        })
    ...
    for bi, batch in enumerate(batches, start=1):
        status.text(f"v14.6.1 {scan_mode}：第 {bi}/{len(batches)} 批（{processed}/{total}）")
    ...
    status.text(
        f"v14.6.1 {scan_mode}完成：掃描 {processed}/{total}｜"
