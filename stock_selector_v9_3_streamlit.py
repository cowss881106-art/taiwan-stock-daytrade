...
if "result_df" in st.session_state:
    ...
    if result_df.empty:
        ...
        for rank, (_, row) in enumerate(top10.iterrows(), start=1):
            ...
            with st.expander(
                f"#{rank}  {code} {name}｜{row['收盤價']:.2f}｜{arrow} {change:+.2f}%｜強度 {int(row['當沖強度'])}",
                expanded=(rank <= 3)
            ):
        ...
        long_candidates = result_df[
            result_df["交易方向"] == "做多"
        ].head(10)

        short_candidates = result_df[
            result_df["交易方向"] == "做空"
        ].head(10)
